...
alloc_proc(void) {
     ...
     */
        ...
        proc->wait_state = 0; // change in lab5, hint: what if wait_state is not initialized?
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    proc->parent = current; //change in lab5, hint: why a process need to know who's its parent?
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        set_links(proc); //change in lab5, hint: why we need process list based on hash(pid)?
    }
    local_intr_restore(intr_flag);
